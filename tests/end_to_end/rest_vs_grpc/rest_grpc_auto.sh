...
for size in "${sizes[@]}"; do
    ...
    PAYLOAD_SIZE="$size" \
    k6 run --out influxdb=http://localhost:8086/k6 k6_rest_auto.js

    echo "=== gRPC test for payload size: $size ==="
    SCENARIO_NAME="grpc_ping_test_${size}" \
    PAYLOAD_SIZE="$size" \
    k6 run --out influxdb=http://localhost:8086/k6 k6_grpc_auto.js
done
...
#     SCENARIO_NAME="rest_ping_test_${size}" \
#     PAYLOAD_SIZE="$size" \
#     k6 run k6_rest_auto.js

#     echo "=== gRPC test for payload size: $size ==="
#     SCENARIO_NAME="grpc_ping_test_${size}" \
#     PAYLOAD_SIZE="$size" \
#     k6 run k6_grpc_auto.js
# done